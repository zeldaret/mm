...
s32 func_8013A240(GlobalContext* globalCtx) {
    ...
    gSaveContext.save.roomInf[3][3] = 0;
    gSaveContext.save.roomInf[3][4] = 0;
...
void func_8013A41C(s32 flag) {
    if (flag < 0x20) {
        gSaveContext.save.roomInf[3][3] |= (1 << flag);
    } else {
        flag &= 0x1F;
        gSaveContext.save.roomInf[3][4] |= (1 << flag);
    }
}
...
u32 func_8013A4C4(s32 flag) {
    ...
    if (flag < 0x20) {
        return saveCtx->save.roomInf[3][3] & (1 << flag);
    } else {
        flag &= 0x1F;
        return saveCtx->save.roomInf[3][4] & (1 << flag);
    }
}
