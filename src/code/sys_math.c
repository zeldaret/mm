/**
 * Maths library: two factorials, integer power, wrappers for libultra's sins and coss (the main ones used), and some random functions moved from OoT's z_actor
 */
#include "global.h"
...
f32 func_80179300(f32 n) {
    ...
    //! @bug Will return 1.0f if the argument truncates to a negative int
    for (i = n; i > 1; i--) {
        ret *= i;
    }
    return ret;
...
f32 func_80179400(s32 n) {
    ...
    //! @bug Will read the array out-of-bounds if the argument is negative
