...
s32 SkelCurve_Update(PlayState* play, SkelAnimeCurve* skelCurve) {
    ...
    skelCurve->animCurFrame += skelCurve->animSpeed * (play->state.framerateDivisor * 0.5f);
...
void SkelCurve_DrawLimb(PlayState* play, s32 limbIndex, SkelAnimeCurve* skelCurve,
    ...
    if (limb->firstChildIdx != LIMB_DONE) {
        SkelCurve_DrawLimb(play, limb->firstChildIdx, skelCurve, overrideLimbDraw, postLimbDraw, lod, thisx);
    }

    Matrix_Pop();

    if (limb->nextLimbIdx != LIMB_DONE) {
        SkelCurve_DrawLimb(play, limb->nextLimbIdx, skelCurve, overrideLimbDraw, postLimbDraw, lod, thisx);
    }
