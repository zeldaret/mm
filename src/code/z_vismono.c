...
void VisMono_UpdateTexture(u16* tex) {
    s32 i;
...
Gfx* VisMono_DrawTexture(Gfx* gfx) {
    s32 y;
    s32 height = 3;
    u16* tex = D_0F000000;
...
void VisMono_Draw(VisMono* this, Gfx** gfxp) {
    ...
    if (this->monoDl) {
        ...
        glistpEnd = VisMono_DrawTexture(monoDL);
    }
...
void VisMono_DrawOld(VisMono* this) {
    ...
    if (!this->tlut) {
        this->tlut = SystemArena_Malloc(256 * sizeof(u16));
        VisMono_UpdateTexture(this->tlut);
    }

    if (!this->monoDl) {
        this->monoDl = SystemArena_Malloc(DLSIZE * sizeof(Gfx));
