...
void Cutscene_Command_Misc(PlayState* play2, CutsceneContext* csCtx, CsCmdBase* cmd) {
    ...
    switch (cmd->base) {
        ...
        case 0x1B:
            if (isStartFrame) {
                play->nextEntrance = ENTRANCE(CUTSCENE, 0, 0);
            ...
            switch (D_801F4DE2) {
                ...
                case 0x44:
                    if (gSaveContext.save.weekEventReg[33] & 0x80) {
                        ...
                        play->nextEntrance = ENTRANCE(MOUNTAIN_VILLAGE_SPRING, 0, 0);
                        gSaveContext.nextCutsceneIndex = 0xFFF0;
                        play->transitionTrigger = TRANS_TRIGGER_START;
                        play->transitionType = TRANS_TYPE_03;
                    }
                ...
                case 0x36:
                    ...
                    play->nextEntrance = ENTRANCE(IKANA_CANYON, 0, 0);
        ...
        switch (cmd->base) {
            ...
            case 2:
                if (INV_CONTENT(ITEM_MASK_KAMARO) == ITEM_MASK_KAMARO) {
                    // Milk bar
                    play->nextEntrance = ENTRANCE(MILK_BAR, 0, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF0;
                } else {
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 0, 0);
            ...
            case 3:
                if (INV_CONTENT(ITEM_MASK_GREAT_FAIRY) == ITEM_MASK_GREAT_FAIRY) {
                    // Fairy's fountain
                    play->nextEntrance = ENTRANCE(FAIRY_FOUNTAIN, 0, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF0;
                } else {
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 1, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 4:
                if (INV_CONTENT(ITEM_MASK_ROMANI) == ITEM_MASK_ROMANI) {
                    // Romani ranch
                    play->nextEntrance = ENTRANCE(ROMANI_RANCH, 0, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF1;
                } else {
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 2, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 5:
                if (INV_CONTENT(ITEM_MASK_BLAST) == ITEM_MASK_BLAST) {
                    // West clock town
                    play->nextEntrance = ENTRANCE(WEST_CLOCK_TOWN, 0, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF0;
                } else {
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 3, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 6:
                if (INV_CONTENT(ITEM_MASK_CIRCUS_LEADER) == ITEM_MASK_CIRCUS_LEADER) {
                    ...
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 5, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 7:
                if (INV_CONTENT(ITEM_MASK_BREMEN) == ITEM_MASK_BREMEN) {
                    ...
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 6, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 9:
                if (INV_CONTENT(ITEM_MASK_COUPLE) == ITEM_MASK_COUPLE) {
                    ...
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 7, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 10:
                if (INV_CONTENT(ITEM_MASK_BUNNY) == ITEM_MASK_BUNNY) {
                    // Cucco shack
                    play->nextEntrance = ENTRANCE(CUCCO_SHACK, 0, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF0;
                } else {
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 4, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
            ...
            case 11:
                if (INV_CONTENT(ITEM_MASK_POSTMAN) == ITEM_MASK_POSTMAN) {
                    // Termina field
                    play->nextEntrance = ENTRANCE(TERMINA_FIELD, 1, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF8;
                } else {
                    // SPOT00
                    play->nextEntrance = ENTRANCE(CUTSCENE, 8, 0);
                    gSaveContext.nextCutsceneIndex = 0xFFF9;
                }
                play->transitionTrigger = TRANS_TRIGGER_START;
                break;
