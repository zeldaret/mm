...
s32 func_800B8500(Actor* actor, PlayState* play, f32 xzRange, f32 yRange, PlayerItemParam exchangeItemId) {
    Player* player = GET_PLAYER(play);

    if ((player->actor.flags & ACTOR_FLAG_100) || ((exchangeItemId > PLAYER_IP_NONE) && Player_InCsMode(play)) ||
        (!actor->isTargeted &&
...
s32 func_800B85E0(Actor* actor, PlayState* play, f32 radius, PlayerItemParam exchangeItemId) {
    return func_800B8500(actor, play, radius, radius, exchangeItemId);
}

s32 func_800B8614(Actor* actor, PlayState* play, f32 radius) {
    return func_800B85E0(actor, play, radius, PLAYER_IP_NONE);
}
