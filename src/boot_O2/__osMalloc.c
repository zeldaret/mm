...
/**
 * @brief Clears the whole \p arena, invalidating every allocated pointer to it.
 * 
 ...
 * If \p size is zero, then an empty region of memory is returned. This region of memory must be freed to avoid memory leaks.
 * 
...
void* __osMalloc(Arena* arena, size_t size) {
    ...
    while (iter != NULL) {
        if (iter->isFree && iter->size >= size) {
            ...
            // If the block is larger than the requested size, then split it and just use the required size of the current block.
 ...
 * 
 * The behaviour is undefined if the pointer \p ptr is not a memory region returned by one of the cited allocating functions.
 ...
 * The pointer \p ptr must be either a pointer previously allocated by `__osMalloc`, `__osMallocR` or `__osRealloc` and not freed yet, or a `NULL` pointer.
 * 
 ...
 * If \p newSize is bigger than the currently allocated allocated pointer, then the area of memory is expanded to a size big enough to fit the requested size.
 * 
 * The behaviour is undefined if the pointer \p ptr is not a memory region returned by one of the cited allocating functions.
 * The behaviour is undefined if the pointer \p ptr doesn't correspond to the given \p arena.
 ...
 * @return void*          On success, the pointer to the reallocated area of memory. On failure, `NULL` and the original parameter \p ptr remains valid.
 */
...
/**
 * @brief Checks the validity of every node of the \p arena.
 * 
 * @param arena  The Arena to check.
