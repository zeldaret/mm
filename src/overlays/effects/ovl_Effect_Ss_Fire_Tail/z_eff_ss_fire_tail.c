...
void EffectSsFireTail_Draw(GlobalContext* globalCtx, u32 index, EffectSs* this) {
    ...
    if (this->actor != NULL) {
        ...
        if (this->rBodyPart < 0) {
            Matrix_InsertTranslation(this->pos.x + this->actor->world.pos.x, this->pos.y + this->actor->world.pos.y,
    ...
    Matrix_RotateY(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) + 0x8000, MTXMODE_APPLY);
    Matrix_InsertZRotation_f(DEG_TO_RAD(temp2 * this->rReg2 * dist), MTXMODE_APPLY);
    ...
    gSPSegment(POLY_XLU_DISP++, 0x08,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0, (globalCtx->state.frames * -20) & 0x1FF,
                                32, 128));
