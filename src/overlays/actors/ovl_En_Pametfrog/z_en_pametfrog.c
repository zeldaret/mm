...
void EnPametfrog_JumpOnGround(EnPametfrog* this, GlobalContext* globalCtx) {
    if (Animation_OnFrame(&this->skelAnime, 1.0f)) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_JUMP);
    } else if (Animation_OnFrame(&this->skelAnime, 11.0f)) {
        EnPametfrog_JumpWaterEffects(this, globalCtx);
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EV_WALK_WATER);
    }
}
...
void EnPametfrog_SetupFallOffSnapper(EnPametfrog* this, GlobalContext* globalCtx) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_DAMAGE);
...
void EnPametfrog_SetupJumpToWall(EnPametfrog* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_VOICE2);
...
void EnPametfrog_SetupWallCrawl(EnPametfrog* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_RUNAWAY);
...
void EnPametfrog_WallCrawl(EnPametfrog* this, GlobalContext* globalCtx) {
    ...
    if (this->freezeTimer > 0) {
        ...
        if (Animation_OnFrame(&this->skelAnime, 15.0f)) {
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_BOMCHU_WALK);
        }

        if (((globalCtx->gameplayFrames % 60) == 0) && (Rand_ZeroOne() < 0.8f)) {
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_REAL);
        }
...
void EnPametfrog_SetupWallPause(EnPametfrog* this) {
    ...
    this->timer = 15;
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_RUNAWAY2);
...
void EnPametfrog_SetupJumpOnSnapper(EnPametfrog* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_GREET);
...
void EnPametfrog_SetupFallInAir(EnPametfrog* this, GlobalContext* globalCtx) {
    ...
    this->timer = 10;
    if (this->actor.colChkInfo.health == 0) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_DEAD);
    } else {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_DAMAGE);
    }
...
void EnPametfrog_MeleeAttack(EnPametfrog* this, GlobalContext* globalCtx) {
    ...
    if ((this->skelAnime.animation == &D_0600347C && Animation_OnFrame(&this->skelAnime, 2.0f)) ||
        ...
        if (this->skelAnime.animation == &D_06003F28) {
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_KICK);
        } else {
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FROG_PUNCH1);
        }
    } else {
        this->collider.base.atFlags &= ~AT_ON;
    }
