...
void EnSlime_Idle(EnSlime* this, PlayState* play) {
    ...
    scale = ((Math_SinF(this->timer * (2 * F_PI / 5)) * (0.13f * timerFactor)) + 1.0f) * 0.01f;
    this->actor.scale.x = scale;
    this->actor.scale.z = scale;
    if (this->timer < 24) {
        this->actor.scale.y = ((Math_CosF(this->timer * (2 * F_PI / 5)) * (0.05f * timerFactor)) + 1.0f) * 0.01f;
    }
...
void EnSlime_Land(EnSlime* this, PlayState* play) {
    ...
    scaleXZ = ((Math_CosF(this->timer * (2 * F_PI / 5)) * (0.05f * scaleY)) + 1.0f) * 0.01f;
    this->actor.scale.x = scaleXZ;
    this->actor.scale.z = scaleXZ;
    if (this->timer < 15) {
        this->actor.scale.y = (1.0f - (Math_SinF(this->timer * (2 * F_PI / 5)) * (0.04f * scaleY))) * 0.01f;
    }
...
void EnSlime_ReactToBluntHit(EnSlime* this, PlayState* play) {
    ...
    if (this->timer < 30) {
        scale = ((Math_CosF(this->timer * (2 * F_PI / 5)) * (0.08f * timerFactor)) + 1.0f) * 0.01f;
        this->actor.scale.x = scale;
        this->actor.scale.z = scale;
    }
    if (this->timer == 15) {
    ...
    this->actor.scale.y = ((Math_SinF((f32)this->timer * (2 * F_PI / 5)) * (0.07f * timerFactor)) + 1.0f) * 0.01f;
    this->actor.shape.rot.x = Rand_CenteredFloat(0x200) * timerFactor;
    this->actor.shape.rot.z = Rand_CenteredFloat(0x200) * timerFactor;
...
void EnSlime_SetupDamaged(EnSlime* this, PlayState* play, s32 arg2) {
    ...
    this->wobbleRot.x = Rand_ZeroOne() * (F_PI * 2);
    this->wobbleRot.z = Rand_ZeroOne() * (F_PI * 2);
...
void EnSlime_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc == EnSlime_Damaged) {
        wobbleScale.x = 1.0f - (Math_SinF((f32)this->timer * (F_PI / 2)) * 0.3f);
        wobbleScale.y = (Math_SinF((f32)this->timer * (F_PI / 2)) * 0.3f) + 1.0f;
        wobbleScale.z = 1.0f - (Math_CosF((f32)this->timer * (F_PI / 2)) * 0.3f);
