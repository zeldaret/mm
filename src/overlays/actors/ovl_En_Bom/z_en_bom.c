...
void func_80871058(EnBom* this, PlayState* play) {
    ...
    if (!(this->actor.bgCheckFlags & 1)) {
        ...
        if ((sp38 < this->actor.speedXZ) ||
            ...
            this->actor.world.rot.y = Math_Atan2S_Depr(sp3C, sp40);
        }
...
void func_808726DC(PlayState* play, Vec3f* arg1, Vec3f* arg2, Vec3f* arg3, s32 arg4) {
    ...
    fuseSegmentPtr->rotY = Math_Atan2S_Depr(spCC.z, spCC.x);
    distXZ = sqrtf(SQXZ(spCC));
    fuseSegmentPtr->rotX = Math_Atan2S_Depr(distXZ, spCC.y);
    ...
    for (i = 0; i < spB0; i++, fuseSegmentPtr++, fuseSegmentPtr2++, arg4 -= 240) {
        ...
        fuseSegmentPtr2->rotY = Math_Atan2S_Depr(spCC.z, spCC.x);
        distXZ = sqrtf(SQXZ(spCC));
        fuseSegmentPtr2->rotX = Math_Atan2S_Depr(distXZ, spCC.y);
