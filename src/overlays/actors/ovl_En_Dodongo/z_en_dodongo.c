...
void func_80877500(EnDodongo* this, GlobalContext* globalCtx) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 19.0f)) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_DODO_J_WALK);
...
void func_8087784C(EnDodongo* this, GlobalContext* globalCtx) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 24.0f)) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_DODO_J_CRY);
    }
...
void func_80877E60(EnDodongo* this, GlobalContext* globalCtx) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 28.0f)) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_DODO_J_EAT);
        if (this->actor.child != NULL) {
...
void func_80878354(EnDodongo* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_DODO_J_TAIL);
...
void func_8087864C(EnDodongo* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_DODO_J_DAMAGE);
    this->timer = 0;
    this->unk_304 = 0;
    this->actor.speedXZ = 0.0f;
...
void func_80878724(EnDodongo* this) {
    ...
    this->timer = 0;
    this->unk_304 = 0;
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_DODO_J_DEAD);
    this->actor.flags &= ~1;
    this->actor.speedXZ = 0.0f;
    Actor_SetColorFilter(&this->actor, 0x4000, 0xFF, 0, 8);
