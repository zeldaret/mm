...
ActorInit Obj_Boyo_InitVars = { ACTOR_OBJ_BOYO,
                                ACTORCAT_PROP,
                                FLAGS,
                                OBJECT_BOYO,
                                sizeof(ObjBoyo),
...
void ObjBoyo_Init(Actor* thisx, PlayState* play) {
    ...
    Collider_SetCylinder(play, &this->unk144, &this->actor, &sCylinderInit);
    Collider_UpdateCylinder(&this->actor, &this->unk144);
    this->actor.colChkInfo.mass = MASS_IMMOVABLE;
    this->unk190 = Lib_SegmentedToVirtual(object_boyo_Matanimheader_000E88);
}
...
Actor* func_809A5E24(ObjBoyo* this, PlayState* play, s32* index) {
    if (this->unk144.base.ocFlags2 & OC2_HIT_PLAYER) {
        *index = 0;
        return (Actor*)GET_PLAYER(play);
    } else if (this->unk144.base.ocFlags1 & OC1_HIT) {
        Actor* hitActor = this->unk144.base.oc;
...
void ObjBoyo_Update(Actor* thisx, PlayState* play2) {
    ...
    if (this->unk194 > 0) {
        ...
        this->actor.scale.x = this->actor.scale.z =
            0.1f + (Math_CosS(this->unk1A8 + this->unk1A4) * this->unk194 * this->unk19C * this->unk198);
        this->actor.scale.y =
            0.1f + (Math_CosS(this->unk1A8 + this->unk1A6) * this->unk194 * this->unk1A0 * this->unk198);
    } else {
        Actor_SetScale(&this->actor, 0.1f);
        if (this->unk144.base.acFlags & AC_HIT) {
            this->unk194 = 30;
            this->unk196 = 2;
    ...
    this->unk144.base.acFlags &= ~AC_HIT;
    this->unk144.base.ocFlags1 &= ~OC1_HIT;
    this->unk144.base.ocFlags2 &= ~OC2_HIT_PLAYER;
    CollisionCheck_SetOC(play, &play->colChkCtx, &this->unk144.base);
    if (this->actor.xzDistToPlayer < 2000.0f) {
        CollisionCheck_SetAC(play, &play->colChkCtx, &this->unk144.base);
    }
}
...
void ObjBoyo_Draw(Actor* thisx, PlayState* play2) {
    ...
    AnimatedMat_Draw(play, this->unk190);
