...
void func_80954340(BgIngate* this, PlayState* play) {
    if (!DECR(this->unk16A)) {
        if (this->unk164 != NULL) {
            ...
            this->unk164 = &play->setupPathList[this->unk164->unk1];
...
void BgIngate_Init(Actor* thisx, PlayState* play2) {
    ...
    if (BgIngate_FindActor(this, play, ACTORCAT_BG, ACTOR_BG_INGATE) == NULL) {
        ...
        this->unk164 = SubS_GetAdditionalPath(play, BGINGATE_GET_FF(&this->dyna.actor), 0);
        ...
        this->unk164 = SubS_GetAdditionalPath(play, BGINGATE_GET_FF(&this->dyna.actor), phi_a2);
        if (this->unk164 != NULL) {
