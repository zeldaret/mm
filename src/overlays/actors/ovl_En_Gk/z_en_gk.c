...
u16 func_80B50410(EnGk* this, PlayState* play) {
    ...
    if (play->sceneNum == SCENE_17SETUGEN2) {
        if (player->transformation == PLAYER_FORM_GORON) {
            if (!(GET_WEEKEVENTREG(WEEKEVENTREG_40_80))) {
                switch (this->unk_31C) {
                    case 0xE7A:
                        return 0xE7B;
                    case 0xE7B:
        ...
        } else if (!(GET_WEEKEVENTREG(WEEKEVENTREG_41_01))) {
            switch (this->unk_31C) {
                case 0xE82:
                    return 0xE83;
                case 0xE83:
        ...
        if (player->transformation == PLAYER_FORM_GORON) {
            if (!(GET_WEEKEVENTREG(WEEKEVENTREG_41_04))) {
                if (this->unk_31C == 0xE88) {
                    if (!(GET_WEEKEVENTREG(WEEKEVENTREG_41_08)) || Inventory_HasEmptyBottle()) {
                        return 0xE89;
                    }
        ...
        if (!(GET_WEEKEVENTREG(WEEKEVENTREG_41_02))) {
            switch (this->unk_31C) {
                case 0xE85:
                    return 0xE86;
                case 0xE86:
...
void func_80B5253C(EnGk* this, PlayState* play) {
    ...
    if (GET_WEEKEVENTREG(WEEKEVENTREG_41_08)) {
        sp24 = 0x93;
    } else {
        sp24 = 0x6A;
    }
...
void EnGk_Init(Actor* thisx, PlayState* play) {
    ...
    } else if (!(GET_WEEKEVENTREG(WEEKEVENTREG_22_04))) {
        this->unk_2E4 = 0;
        this->unk_318 = this->actor.cutscene;
        this->actor.flags |= ACTOR_FLAG_10;
...
void EnGk_Update(Actor* thisx, PlayState* play) {
    ...
    if ((ENGK_GET_F(&this->actor) == ENGK_F_1) ||
        ((ENGK_GET_F(&this->actor) == ENGK_F_0) && !(GET_WEEKEVENTREG(WEEKEVENTREG_22_04)))) {
...
void EnGk_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((ENGK_GET_F(&this->actor) == ENGK_F_0) && (GET_WEEKEVENTREG(WEEKEVENTREG_22_04))) {
