...
void func_80B5227C(EnGk* this, PlayState* play) {
    ...
    if ((sp26 + 1) == lastFrame) {
        ...
        if (!(this->unk_1E4 & 0x80)) {
            SET_WEEKEVENTREG(WEEKEVENTREG_CALMED_GORON_ELDER_SON);
        }
...
void EnGk_Update(Actor* thisx, PlayState* play) {
    ...
    if ((ENGK_GET_F(&this->actor) == ENGK_F_1) ||
        ((ENGK_GET_F(&this->actor) == ENGK_F_0) && !CHECK_WEEKEVENTREG(WEEKEVENTREG_CALMED_GORON_ELDER_SON))) {
...
void EnGk_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((ENGK_GET_F(&this->actor) == ENGK_F_0) && CHECK_WEEKEVENTREG(WEEKEVENTREG_CALMED_GORON_ELDER_SON)) {
