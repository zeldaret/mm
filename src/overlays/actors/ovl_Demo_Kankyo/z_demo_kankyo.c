...
void DemoKakyo_LostWoodsSparkleActionFunc(DemoKankyo* this, PlayState* play) {
    ...
    } else if (play->envCtx.unk_F2[3] < DEMOKANKYO_PARTICLE_COUNT) {
    ...
    for (i = 0; i < play->envCtx.unk_F2[3]; i++) {
        ...
        switch (this->particles[i].state) {
            case DEMO_KANKYO_STATE_INIT:
                this->particles[i].posBase.x = play->view.eye.x + (eyeToAtNormX * 80.0f);
                this->particles[i].posBase.y = play->view.eye.y + (eyeToAtNormY * 80.0f);
                this->particles[i].posBase.z = play->view.eye.z + (eyeToAtNormZ * 80.0f);
...
void DemoKakyo_MoonSparklesActionFunc(DemoKankyo* this, PlayState* play) {
    ...
    if (play->envCtx.unk_F2[3] < DEMOKANKYO_PARTICLE_COUNT) {
        play->envCtx.unk_F2[3] += 16;
    }
    ...
    for (i = 0; i < play->envCtx.unk_F2[3]; i++) {
        switch (this->particles[i].state) {
            case DEMO_KANKYO_STATE_INIT:
                this->particles[i].posBase.x = play->view.eye.x + (eyeToAtNormX * halfScreenHeight);
                this->particles[i].posBase.y = play->view.eye.y + (eyeToAtNormY * halfScreenHeight);
                this->particles[i].posBase.z = play->view.eye.z + (eyeToAtNormZ * halfScreenHeight);
