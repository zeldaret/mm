...
void EnZow_PostLimbDraw1(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    if (limbIndex == 15) {
        Matrix_MultVec3f(&D_80BDDD4C, &thisx->focus.pos);
    }
}

void EnZow_PostLimbDraw2(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    EnZow_PostLimbDraw1(play, limbIndex, dList, rot, thisx);
}
    ...
    if (this->unk_2CE != 0) {
        ...
        if (this->unk_2CE >= 255) {
            ...
            SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable,
                                  this->skelAnime.dListCount, NULL, EnZow_PostLimbDraw1, &this->actor);
        } else {
            gSPSegment(POLY_XLU_DISP++, 0x08, Lib_SegmentedToVirtual(sp54[this->unk_2C4]));

            func_800BDAA0(play, &this->skelAnime, NULL, EnZow_PostLimbDraw2, &this->actor, this->unk_2CE);
        }
