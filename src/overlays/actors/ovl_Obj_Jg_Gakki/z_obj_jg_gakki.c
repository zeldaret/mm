...
void ObjJgGakki_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    f32 frameCount = Animation_GetLastFrame(&object_jg_Anim_01B1E8);

    ActorShape_Init(&this->actor.shape, 0.0f, ActorShadow_DrawCircle, 24.0f);
    SkelAnime_Init(globalCtx, &this->skelAnime, &object_jg_Skel_01B210, NULL, NULL, NULL, 0);

    if (((globalCtx->sceneNum == SCENE_SPOT00) && (gSaveContext.sceneSetupIndex == 7)) &&
        (globalCtx->csCtx.unk_12 == 0)) {
        Animation_Change(&this->skelAnime, &object_jg_Anim_01B1E8, 1.0f, frameCount, frameCount, 2, 0.0f);
    } else if ((globalCtx->sceneNum == SCENE_17SETUGEN) || (globalCtx->sceneNum == SCENE_10YUKIYAMANOMURA)) {
        Animation_Change(&this->skelAnime, &object_jg_Anim_01B1E8, 1.0f, 0.0f, frameCount, 2, 0.0f);
    } else {
