...
void func_80B3E168(EnDaiParticle* particle, PlayState* play2) {
    ...
    for (i = 0; i < 32; i++, particle++) {
        if (particle->isEnabled == true) {
            ...
            gSPSegment(POLY_XLU_DISP++, 0x08,
                       Gfx_TwoTexScroll(play->state.gfxCtx, 0, (particle->unk_02 + (i * 3)) * 3,
...
s32 func_80B3E5B4(EnDai* this, PlayState* play) {
    func_80B3E168(this->particles, play);
    return 0;
}
...
s32 func_80B3E69C(EnDai* this, PlayState* play) {
    ...
    if ((play->csCtx.state != 0) && (play->sceneNum == SCENE_12HAKUGINMAE) &&
        (play->csCtx.currentCsIndex == 0) && !(gSaveContext.save.weekEventReg[30] & 1)) {
