...
void func_80962340(EnFu* this, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, &play->state)) {
        if (this->unk_54A == 2) {
            ...
            } else if ((gSaveContext.timerTimeRemaining[TIMER_ID_MINIGAME_2] == SECONDS_TO_TIMER(0)) &&
                       (this->unk_552 != 0x2888)) {
...
void func_80962F4C(EnFu* this, PlayState* play) {
    ...
    if (gSaveContext.timerTimeRemaining[TIMER_ID_MINIGAME_2] < SECONDS_TO_TIMER(20)) {
        s16 val = D_80964B00[this->unk_542] + 200;

        Math_SmoothStepToS(&fuKaiten->rotationSpeed, val, 10, 5, 5);
    } else if (gSaveContext.timerTimeRemaining[TIMER_ID_MINIGAME_2] < SECONDS_TO_TIMER(40)) {
    ...
    if ((!DynaPolyActor_IsInRidingRotatingState((DynaPolyActor*)this->actor.child) &&
        ...
        (gSaveContext.timerTimeRemaining[TIMER_ID_MINIGAME_2] <= SECONDS_TO_TIMER(0)) ||
        (this->unk_548 == this->unk_54C)) {
        player->stateFlags3 &= ~0x400000;
        func_80961E88(play);
        player->stateFlags1 |= 0x20;
        if (this->unk_548 < this->unk_54C) {
            if (gSaveContext.timerTimeRemaining[TIMER_ID_MINIGAME_2] == SECONDS_TO_TIMER(0)) {
