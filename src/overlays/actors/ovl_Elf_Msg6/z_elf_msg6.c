...
s32 func_80BA15A0(void) {
    if (CHECK_QUEST_ITEM(QUEST_REMAINS_ODOWLA) && !(GET_WEEKEVENTREG(WEEKEVENTREG_87_10))) {
        return true;
    }

    if (CHECK_QUEST_ITEM(QUEST_REMAINS_GOHT) && !(GET_WEEKEVENTREG(WEEKEVENTREG_87_20))) {
        return true;
    }

    if (CHECK_QUEST_ITEM(QUEST_REMAINS_GYORG) && !(GET_WEEKEVENTREG(WEEKEVENTREG_87_40))) {
        return true;
    }

    if (CHECK_QUEST_ITEM(QUEST_REMAINS_TWINMOLD) && !(GET_WEEKEVENTREG(WEEKEVENTREG_87_80))) {
        return true;
    }
...
s32 func_80BA16F4(ElfMsg6* this, PlayState* play) {
    if (!(GET_WEEKEVENTREG(WEEKEVENTREG_31_04)) && (INV_CONTENT(ITEM_MASK_DEKU) == ITEM_MASK_DEKU)) {
    ...
    if (!CHECK_QUEST_ITEM(QUEST_REMAINS_GOHT)) {
        if (GET_WEEKEVENTREG(WEEKEVENTREG_31_01)) {
    ...
    if (!CHECK_QUEST_ITEM(QUEST_REMAINS_GYORG)) {
        if (GET_WEEKEVENTREG(WEEKEVENTREG_31_02)) {
    ...
    if (!CHECK_QUEST_ITEM(QUEST_REMAINS_TWINMOLD)) {
        if (GET_WEEKEVENTREG(WEEKEVENTREG_80_04)) {
    ...
    switch (ELFMSG6_GET_F(&this->actor)) {
        ...
        case 1:
            ...
            switch (ELFMSG6_GET_F0(&this->actor)) {
                ...
                case 1:
                    if (GET_WEEKEVENTREG(WEEKEVENTREG_83_02)) {
                        Actor_MarkForDeath(&this->actor);
                        return;
                    }
                    break;
        ...
        case 2:
            ...
            if (GET_WEEKEVENTREG(WEEKEVENTREG_08_40)) {
                ...
                if (!(GET_WEEKEVENTREG(WEEKEVENTREG_74_20)) || (GET_WEEKEVENTREG(WEEKEVENTREG_79_10))) {
                    Actor_MarkForDeath(&this->actor);
                    return;
                }
        ...
        case 3:
            if (((ELFMSG6_SWITCHFLAG(&this->actor) != 0x7F) &&
                 Flags_GetSwitch(play, ELFMSG6_SWITCHFLAG(&this->actor))) ||
                (GET_WEEKEVENTREG(WEEKEVENTREG_88_10)) || (GET_WEEKEVENTREG(WEEKEVENTREG_91_01)) ||
...
void func_80BA1CF8(ElfMsg6* this, PlayState* play) {
    ...
    if ((this->actor.textId == 0x224) && (GET_WEEKEVENTREG(WEEKEVENTREG_08_40))) {
        this->actor.textId = 0x25B;
...
void func_80BA1F80(ElfMsg6* this, PlayState* play) {
    if (((ELFMSG6_GET_F0(&this->actor)) == 1) && (GET_WEEKEVENTREG(WEEKEVENTREG_83_02))) {
        Actor_MarkForDeath(&this->actor);
        return;
    }
