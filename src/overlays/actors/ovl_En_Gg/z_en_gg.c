...
s32 func_80B34FB4(EnGg* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer < 250.0f) && (this->actor.xzDistToPlayer > 50.0f) &&
        (CHECK_FLAG_ALL(this->actor.flags, ACTOR_FLAG_REACT_TO_LENS) || (gSaveContext.save.weekEventReg[19] & 0x80))) {
...
void EnGg_Update(Actor* thisx, PlayState* play) {
    ...
    if (!(gSaveContext.save.weekEventReg[91] & 0x10) &&
        ((gSaveContext.save.weekEventReg[19] & 0x80) || CHECK_FLAG_ALL(this->actor.flags, ACTOR_FLAG_REACT_TO_LENS) ||
         (this->unk_308 == 1))) {
