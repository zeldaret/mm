    ...
    switch (this->curTextId) {
        ...
        case 0x455: // you deposited a lot
            if (this->isNewAccount == true) {
                ...
                play->msgCtx.bankRupees = HS_GET_BANK_RUPPEES();
        ...
        case 0x45A:
            if ((HS_GET_BANK_RUPPEES() >= 200) && (this->previousBankValue < 200) &&
            ...
            } else if ((HS_GET_BANK_RUPPEES() >= 1000) && ((this->previousBankValue) < 1000) &&
            ...
            } else if (HS_GET_BANK_RUPPEES() >= 5000) {
                ...
                } else if (this->previousBankValue < (s16)HS_GET_BANK_RUPPEES()) {
                    Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, GINKO_ANIM_SITTING);
...
void EnGinkoMan_WaitForDialogueInput(EnGinkoMan* this, PlayState* play) {
    ...
    switch (this->curTextId) {
        ...
        case 0x452:
            if (play->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if (gSaveContext.save.saveInfo.playerData.rupees < play->msgCtx.bankRupeesSelected) {
                    ...
                    this->previousBankValue = HS_GET_BANK_RUPPEES();
                    HS_SET_BANK_RUPPEES(HS_GET_BANK_RUPPEES() + play->msgCtx.bankRupeesSelected);
                }
            } else { // GINKOMAN_CHOICE_NO
                Audio_PlaySfx_MessageCancel();
                Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, GINKO_ANIM_SITTING);
        ...
        case 0x471:
            if (play->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if ((s32)HS_GET_BANK_RUPPEES() < (play->msgCtx.bankRupeesSelected + this->serviceFee)) {
                    Audio_PlaySfx(NA_SE_SY_ERROR);
                    Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, GINKO_ANIM_LEGSMACKING);
                    ...
                    this->previousBankValue = HS_GET_BANK_RUPPEES();
                    HS_SET_BANK_RUPPEES(HS_GET_BANK_RUPPEES() - play->msgCtx.bankRupeesSelected - this->serviceFee);
                    Rupees_ChangeBy(play->msgCtx.bankRupeesSelected);
                }
            } else {
                Audio_PlaySfx_MessageCancel();
