...
void EnGinkoMan_Idle(EnGinkoMan* this, GlobalContext* globalCtx) {
    ...
    if (func_800B84D0(&this->actor, globalCtx)) { // Listen for dialogue start?
        if ((gSaveContext.save.roomInf[7][0] & 0xFFFF) == 0) {
    ...
    switch (this->curTextId) {
        ...
        case 0x455: // you deposited a lot
            if (this->isNewAccount == true) {
                ...
                globalCtx->msgCtx.bankRupees = gSaveContext.save.roomInf[7][0] & 0xFFFF;
        ...
        case 0x45A: // "All right, little guy, now I've got a total of [rupees] from you!"
            if (((gSaveContext.save.roomInf[7][0] & 0xFFFF) >= 200) && (this->previousBankValue < 200) &&
            ...
            } else if (((gSaveContext.save.roomInf[7][0] & 0xFFFF) >= 1000) && ((this->previousBankValue) < 1000) &&
            ...
            } else if ((gSaveContext.save.roomInf[7][0] & 0xFFFF) >= 5000) {
                ...
                } else if (this->previousBankValue < (s16)(gSaveContext.save.roomInf[7][0] & 0xFFFF)) {
                    func_800BDC5C(&this->skelAnime, animations, GINKO_SITTING);
        ...
        case 0x47E: // "Your deposits total [rupees]."
            if (this->choiceDepositWithdrawl == GINKOMAN_CHOICE_DEPOSIT) {
                if ((u32)(gSaveContext.save.roomInf[7][0] & 0xFFFF) >= 5000) {
    ...
    switch (this->curTextId) {
        case 0x44E: // "...So, what'll it be?
            if (globalCtx->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if ((gSaveContext.save.roomInf[7][0] & 0xFFFF) >= 5000) {
        ...
        case 0x452: // Really? are you really depositing rupees?
            if (globalCtx->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if (gSaveContext.save.playerData.rupees < globalCtx->msgCtx.bankRupeesSelected) {
                    ...
                    this->previousBankValue = gSaveContext.save.roomInf[7][0] & 0xFFFF;
                    if (1) {} // Needed to match
                    gSaveContext.save.roomInf[7][0] =
                        (gSaveContext.save.roomInf[7][0] & 0xFFFF0000) |
                        ((gSaveContext.save.roomInf[7][0] & 0xFFFF) + globalCtx->msgCtx.bankRupeesSelected);
                }
            } else { // GINKOMAN_CHOICE_NO
                func_8019F230();
                func_800BDC5C(&this->skelAnime, animations, GINKO_SITTING);
        ...
        case 0x471: // Are you really withdrawling [selected rupees]?
            if (globalCtx->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if ((s32)((gSaveContext.save.roomInf[7][0] & 0xFFFF)) <
                    ...
                    this->previousBankValue = (s16)(gSaveContext.save.roomInf[7][0] & 0xFFFF);
                    gSaveContext.save.roomInf[7][0] =
                        (gSaveContext.save.roomInf[7][0] & 0xFFFF0000) |
                        (((gSaveContext.save.roomInf[7][0] & 0xFFFF) - globalCtx->msgCtx.bankRupeesSelected) -
                         this->serviceFee);
