...
void EnGinkoMan_SetupIdle(EnGinkoMan* this) {
    ...
    Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimations, GINKO_ANIM_SITTING);
...
void EnGinkoMan_Idle(EnGinkoMan* this, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, &play->state)) {
        if ((gSaveContext.save.bankRupees & 0xFFFF) == 0) {
            Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimations, GINKO_ANIM_FLOORSMACKING);
...
void EnGinkoMan_SetupStamp(EnGinkoMan* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimations, GINKO_ANIM_REACHING);
    ...
    if (this->actor.xzDistToPlayer > 160.0f) {
        if (this->animTimer == 0) {
            if (this->skelAnime.animation != &object_boj_Anim_004A7C) {
                this->animTimer = 40;
                Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimations, GINKO_ANIM_ADVERTISING);
            }
        }
    } else if ((this->animTimer == 0) && (this->skelAnime.animation != &object_boj_Anim_000AC4)) {
        this->animTimer = 40;
        Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimations, GINKO_ANIM_AMAZED);
    }
