...
void EnGinkoMan_SetupIdle(EnGinkoMan* this) {
    ...
    Actor_ChangeAnimationByInfo(&this->skelAnime, animations, GINKO_SITTING);
...
void EnGinkoMan_Idle(EnGinkoMan* this, GlobalContext* globalCtx) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, &globalCtx->state)) {
        if ((gSaveContext.roomInf[127][0] & 0xFFFF) == 0) {
            Actor_ChangeAnimationByInfo(&this->skelAnime, animations, GINKO_FLOORSMACKING);
...
void EnGinkoMan_SetupStamp(EnGinkoMan* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, animations, GINKO_REACHING);
    ...
    if (this->actor.xzDistToPlayer > 160.0f) {
        if (this->animTimer == 0) {
            if (this->skelAnime.animation != &D_06004A7C) {
                this->animTimer = 40;
                Actor_ChangeAnimationByInfo(&this->skelAnime, animations, GINKO_ADVERTISING);
            }
        }
    } else if ((this->animTimer == 0) && (this->skelAnime.animation != &D_06000AC4)) {
        this->animTimer = 40;
        Actor_ChangeAnimationByInfo(&this->skelAnime, animations, GINKO_AMAZED);
    }
