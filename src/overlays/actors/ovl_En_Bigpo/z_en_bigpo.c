...
void EnBigpo_UpdateSpin(EnBigpo* this) {
    ...
    if ((oldYaw < 0) && (this->actor.shape.rot.y > 0)) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_PO_ROLL); // spinning sfx during spin attack
    }
}
...
void EnBigpo_SpawnCutsceneStage5(EnBigpo* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_STALKIDS_APPEAR);
...
void EnBigpo_SetupWarpOut(EnBigpo* this) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_PO_DISAPPEAR);
...
void EnBigpo_BurnAwayDeath(EnBigpo* this, GlobalContext* globalCtx) {
    ...
    if (this->idleTimer == 18) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_WIZ_DISAPPEAR);
    }
}
...
void EnBigpo_SpawnScoopSoul(EnBigpo* this) {
    ...
    this->savedHeight = this->actor.world.pos.y;
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EV_METAL_BOX_BOUND); // misnamed?
...
void EnBigpo_ScoopSoulIdle(EnBigpo* this, GlobalContext* globalCtx) {
    ...
    if (Actor_HasParent(&this->actor, globalCtx)) {
        ...
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_PO_LAUGH);
...
s32 EnBigpo_ApplyDamage(EnBigpo* this, GlobalContext* globalCtx) {
    if ((this->collider.base.acFlags & AC_HIT) && !(this->collider.base.acFlags & AC_HARD)) {
        ...
        if (Actor_ApplyDamage(&this->actor) == 0) {
            this->actor.flags &= ~0x1; // targetable OFF
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_PO_DEAD);
            Enemy_StartFinishingBlow(globalCtx, &this->actor);
            if (this->actor.params == ENBIGPO_SUMMONED) { // dampe type
                func_801A2ED8();
            }
        } else {
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_PO_DAMAGE);
        }
