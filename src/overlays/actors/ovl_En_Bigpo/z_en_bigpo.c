...
void EnBigpo_BurnAwayDeath(EnBigpo* this, GlobalContext* globalCtx) {
    ...
    this->idleTimer++;
    if (this->idleTimer < 8) {
        camYaw = Camera_GetCamDirYaw(ACTIVE_CAM) + 0x4800;
        if (this->idleTimer < 5) {
...
void EnBigpo_DrawScoopSoul(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_RotateY(BINANG_ROT180(Camera_GetCamDirYaw(ACTIVE_CAM)), MTXMODE_APPLY);
...
void EnBigpo_DrawCircleFlames(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_8012C2DC(globalCtx->state.gfxCtx);
    Matrix_RotateY(BINANG_ROT180(Camera_GetCamDirYaw(ACTIVE_CAM)), MTXMODE_NEW);
    if (this->actionFunc == EnBigpo_SpawnCutsceneStage6) {
