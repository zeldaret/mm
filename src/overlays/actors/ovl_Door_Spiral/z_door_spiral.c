...
void DoorSpiral_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.room != globalCtx->doorCtx.actorList[transition].sides[0].room) {
...
void DoorSpiral_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    globalCtx->doorCtx.actorList[transition].id *= -1;
}
...
void DoorSpiral_Wait(DoorSpiral* this, GlobalContext* globalCtx) {
    ...
    if (this->shouldClimb) {
        ...
        player->doorNext = ((u16)globalCtx->doorCtx.actorList[transition].params) >> 10;
