...
s32 func_80BE06DC(EnTab* this, PlayState* play) {
    ...
    if (((this->unk_2FC & 7) != SUBS_OFFER_MODE_NONE) && Actor_ProcessTalkRequest(&this->actor, &play->state)) {
        SubS_UpdateFlags(&this->unk_2FC, SUBS_OFFER_MODE_NONE, SUBS_OFFER_MODE_MAX);
        ret = true;
        this->unk_320 = 0;
        this->unk_328 = NULL;
...
s32 func_80BE0F04(EnTab* this, PlayState* play, ScheduleOutput* scheduleOutput) {
    ...
    if (sp28) {
        ...
        SubS_UpdateFlags(&this->unk_2FC, SUBS_OFFER_MODE_ONSCREEN, SUBS_OFFER_MODE_MAX);
...
void EnTab_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_1D8 != 0) {
        ...
        SubS_OfferItem(&this->actor, play, radius, height, PLAYER_IA_NONE, this->unk_2FC & SUBS_OFFER_MODE_MAX);
