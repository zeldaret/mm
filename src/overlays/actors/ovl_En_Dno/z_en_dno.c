...
void func_80A714B4(EnDno* this, GlobalContext* globalCtx) {
    ...
    do {
        actor = SubS_ActorFind(globalCtx, actor, ACTORCAT_BG, ACTOR_BG_CRACE_MOVEBG);
        if (actor != NULL) {
...
void func_80A715DC(EnDno* this, GlobalContext* globalCtx) {
    ...
    do {
        crace = (BgCraceMovebg*)SubS_ActorFind(globalCtx, &crace->actor, ACTORCAT_BG, ACTOR_BG_CRACE_MOVEBG);
        if (crace != NULL) {
...
void EnDno_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    while (true) {
        actor = SubS_ActorFind(globalCtx, actor, ACTORCAT_NPC, ACTOR_EN_DNO);
        if (actor != NULL) {
            if (actor != thisx) {
            ...
            switch (ENDNO_GET_C000(thisx)) {
                ...
                case ENDNO_GET_C000_1:
                    if (gSaveContext.weekEventReg[23] & 0x20) {
                        ...
                        this->unk_460 = SubS_ActorFind(globalCtx, NULL, ACTORCAT_NPC, ACTOR_EN_DNQ);
