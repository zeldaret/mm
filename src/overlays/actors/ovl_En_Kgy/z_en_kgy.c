...
void func_80B40C74(PlayState* play) {
    ...
    if (CURRENT_DAY == 1) {
        gSaveContext.save.permanentSceneFlags[play->sceneNum].floors |= 2;
    } else {
        gSaveContext.save.permanentSceneFlags[play->sceneNum].floors &= ~2;
    }
}
...
void func_80B40D00(PlayState* play) {
    gSaveContext.save.permanentSceneFlags[play->sceneNum].floors |= 4;
}
...
s32 func_80B40D8C(PlayState* play) {
    return gSaveContext.save.permanentSceneFlags[play->sceneNum].floors & 4;
}
