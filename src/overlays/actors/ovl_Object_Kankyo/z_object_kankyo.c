...
void func_808DD3C8(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((globalCtx->cameraPtrs[CAM_ID_MAIN]->stateFlags & CAM_STATE_8) || ((u8)globalCtx->envCtx.unk_E2 == 0)) {
        return;
    }
...
void func_808DD970(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (globalCtx->sceneNum == SCENE_KYOJINNOMA) {
        ...
        if (!(globalCtx->cameraPtrs[CAM_ID_MAIN]->stateFlags & CAM_STATE_8) || (phi_f26 == 0.0f)) {
            return;
        }
    }
