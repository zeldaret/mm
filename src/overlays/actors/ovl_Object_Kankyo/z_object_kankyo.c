...
void func_808DD3C8(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((globalCtx->cameraPtrs[CAM_ID_MAIN]->stateFlags & CAM_STATE_UNDERWATER) ||
        ((u8)globalCtx->envCtx.unk_E2 == 0)) {
        return;
    }
    ...
    for (i = 0; i < sp68; i++) {
        ...
        Play_GetScreenPos(globalCtx, &worldPos, &screenPos);
        ...
        if ((screenPos.x >= 0.0f) && (screenPos.x < SCREEN_WIDTH) && (screenPos.y >= 0.0f) &&
            ...
            temp_f2 = Math_Vec3f_DistXYZ(&worldPos, &globalCtx->view.eye) / 300.0f;
...
void func_808DD970(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (globalCtx->sceneNum == SCENE_KYOJINNOMA) {
        ...
        if (!(globalCtx->cameraPtrs[CAM_ID_MAIN]->stateFlags & CAM_STATE_UNDERWATER) || (phi_f26 == 0.0f)) {
            return;
        }
    }
    ...
    for (i = 0; i < this->unk_114C; i++) {
        ...
        if ((screenPos.x >= 0.0f) && (screenPos.x < SCREEN_WIDTH) && (screenPos.y >= 0.0f) &&
            ...
            temp_f0 = Math_Vec3f_DistXYZ(&worldPos, &globalCtx->view.eye);
