...
void EnMaYts_Init(Actor* thisx, PlayState* play) {
    ...
    } else if (CURRENT_DAY == 2 && gSaveContext.save.isNight == 1 && (GET_WEEKEVENTREG(WEEKEVENTREG_22_01))) {
...
void EnMaYts_StartDialogue(EnMaYts* this, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, &play->state)) {
        if (!(gSaveContext.save.playerForm == PLAYER_FORM_HUMAN)) {
            if (!(GET_WEEKEVENTREG(WEEKEVENTREG_65_80))) {
            ...
            if (!(GET_WEEKEVENTREG(WEEKEVENTREG_65_40))) {
        ...
        } else if (!(GET_WEEKEVENTREG(WEEKEVENTREG_21_20))) {
            EnMaYts_SetFaceExpression(this, 0, 0);
            Message_StartTextbox(play, 0x3367, &this->actor);
            this->textId = 0x3367;
        } else {
            if (!(GET_WEEKEVENTREG(WEEKEVENTREG_65_20))) {
