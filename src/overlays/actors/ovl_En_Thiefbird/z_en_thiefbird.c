...
void EnThiefbird_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    } else if ((gSaveContext.save.roomInf[6][5] & 0xFF000000) >> 0x18) {
        Actor_MarkForDeath(&this->actor);
    } else {
        func_80C11538(this);
    }
...
s32 func_80C10B0C(EnThiefbird* this, GlobalContext* globalCtx) {
    ...
    for (; slotId < ARRAY_COUNT(gSaveContext.save.inventory.items); slotId++) {
    ...
    if (!((gSaveContext.save.roomInf[6][5] & 0xFF000000) >> 0x18)) {
        gSaveContext.save.roomInf[6][5] = (gSaveContext.save.roomInf[6][5] & 0xFFFFFF) | ((itemId1 & 0xFF) << 0x18);
    } else {
        gSaveContext.save.roomInf[6][5] = (gSaveContext.save.roomInf[6][5] & 0xFF00FFFF) | ((itemId1 & 0xFF) << 0x10);
    }
        ...
        if (this->collider.base.atFlags & AT_HIT) {
            ...
            if (!(this->collider.base.atFlags & AT_BOUNCED)) {
                if ((D_80C1392C != 0) && CUR_UPG_VALUE(UPG_QUIVER) &&
                    (!((gSaveContext.save.roomInf[6][5] & 0xFF000000) >> 0x18) ||
                     !((gSaveContext.save.roomInf[6][5] & 0xFF0000) >> 0x10)) &&
