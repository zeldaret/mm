...
void EnOsn_Init(Actor* thisx, PlayState* play) {
    ...
    switch (ENOSN_GET_3(&this->actor)) {
        case 0:
            if (((gSaveContext.save.entrance == ENTRANCE(CLOCK_TOWER_INTERIOR, 2, 0)) ||
                 (gSaveContext.save.entrance == ENTRANCE(CLOCK_TOWER_INTERIOR, 3, 0))) ||
                (gSaveContext.save.entrance == ENTRANCE(CLOCK_TOWER_INTERIOR, 6, 0))) {
                this->unk_1EA |= 1;
            }
            this->unk_1F0 = 1;
            if (play->sceneNum == SCENE_INSIDETOWER) {
                ...
                if (gSaveContext.save.entrance == ENTRANCE(CLOCK_TOWER_INTERIOR, 3, 0)) {
                    func_80AD1398(this);
                    this->actionFunc = func_80AD1634;
                    return;
                }
