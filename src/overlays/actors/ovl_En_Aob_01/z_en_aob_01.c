...
s32 EnAob01_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (limbIndex == MAMAMU_YAN_LIMB_HEAD) {
        ...
        Matrix_RotateXS(this->unk_2DA.y, MTXMODE_APPLY);
        Matrix_RotateZS(this->unk_2DA.x * -1, MTXMODE_APPLY);
        Matrix_Translate(-1500.0f, 0.0f, 0.0f, MTXMODE_APPLY);
    }

    if (limbIndex == MAMAMU_YAN_LIMB_TORSO) {
        Matrix_RotateXS(this->unk_2E0.y * -1, MTXMODE_APPLY);
        Matrix_RotateZS(this->unk_2E0.x * -1, MTXMODE_APPLY);
    }
