...
void EnMa4_Init(Actor* thisx, PlayState* play) {
    ...
    if (Cutscene_GetSceneSetupIndex(play) != 0) { // if (sceneSetupIndex != 0)
        ...
        if (gSaveContext.save.entrance == ENTRANCE(ROMANI_RANCH, 1, 0)) {
            EnMa4_ChangeAnim(this, 0);
            this->state = MA4_STATE_AFTERHORSEBACKGAME;
        } else if (gSaveContext.save.entrance == ENTRANCE(ROMANI_RANCH, 10, 0)) {
            EnMa4_ChangeAnim(this, 0);
...
void EnMa4_HorsebackGameEnd(EnMa4* this, PlayState* play) {
    ...
    if (sFrameCounter == 25) {
        ...
        play->nextEntrance = ENTRANCE(ROMANI_RANCH, 1, 0);
