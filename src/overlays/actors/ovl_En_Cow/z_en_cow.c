...
void EnCow_CheckForEmptyBottle(EnCow* this, GlobalContext* globalCtx) {
    if ((func_80152498(&globalCtx->msgCtx) == 5) && func_80147624(globalCtx)) {
        if (func_80114E90()) {
            func_80151938(globalCtx, 0x32C9);
            this->actionFunc = EnCow_GiveMilk;
        } else {
            func_80151938(globalCtx, 0x32CA);
...
void EnCow_Talk(EnCow* this, GlobalContext* globalCtx) {
    if (func_800B84D0(&this->actor, globalCtx)) {
        if (this->actor.textId == 0x32C8) {
            this->actionFunc = EnCow_CheckForEmptyBottle;
        } else if (this->actor.textId == 0x32C9) {
            this->actionFunc = EnCow_GiveMilk;
        } else {
            this->actionFunc = EnCow_TalkEnd;
        }
    ...
    if (this->actor.xzDistToPlayer < 150.0f &&
        ...
        if (func_801A5100() == 4) {
            if (!(gSaveContext.weekEventReg[87] & 1)) {
                ...
                if (func_80114E90() != 0) {
                    this->actor.textId = 0x32C9;
                } else {
                    this->actor.textId = 0x32CA;
                }
                this->actor.flags |= 0x10000;
                func_800B8614(&this->actor, globalCtx, 170.0f);
                this->actionFunc = EnCow_Talk;
...
void EnCow_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    CollisionCheck_SetOC(globalCtx, &globalCtx2->colChkCtx, &this->colliders[0].base);
    CollisionCheck_SetOC(globalCtx, &globalCtx2->colChkCtx, &this->colliders[1].base);
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        if (this->skelAnime.animation == &gCowBodyChewAnim) {
            Audio_PlayActorSound2(&this->actor, 0x28DF);
