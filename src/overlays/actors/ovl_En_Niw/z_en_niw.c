...
void EnNiw_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->niwType == ENNIW_TYPE_UNK2) {
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EV_CHICKEN_CRY_M); // crow
        this->sfxTimer1 = 30;
        this->unkTimer250 = 30;
...
void EnNiw_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.floorHeight <= BGCHECK_Y_MIN || this->actor.floorHeight >= BGCHECK_Y_MAX) {
        ...
        if ((this->flutterSfxTimer == 0) && (this->unknownState28E == 4)) {
            ...
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_CHICKEN_FLUTTER);
        }

        if (this->sfxTimer1 == 0) {
            if (this->unknownState28E != 0) {
                this->sfxTimer1 = 30;
                Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EV_CHICKEN_CRY_A); // attack cluck
            } else {
                this->sfxTimer1 = 300;
                Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EV_CHICKEN_CRY_N); // cluck
            }
        }
