...
void EnBomBowlMan_Init(Actor* thisx, PlayState* play) {
    ...
    if ((gSaveContext.save.entrance == ENTRANCE(EAST_CLOCK_TOWN, 2, 0)) &&
        (gSaveContext.save.weekEventReg[73] & 0x80) && !CHECK_QUEST_ITEM(QUEST_BOMBERS_NOTEBOOK)) {
...
void func_809C51B4(EnBomBowlMan* this, PlayState* play) {
    ...
    if ((play->msgCtx.unk120B1 == 0) &&
        ...
        play->nextEntrance = Entrance_CreateIndexFromSpawn(6);
