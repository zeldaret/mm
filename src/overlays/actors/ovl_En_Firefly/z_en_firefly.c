...
void EnFirefly_SetupFall(EnFirefly* this, GlobalContext* globalCtx) {
    ...
    Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FFLY_DEAD);
...
void EnFirefly_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (this->collider.base.atFlags & AT_HIT) {
        ...
        Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FFLY_ATTACK);
    ...
    if (this->actor.colChkInfo.health != 0) {
        ...
        if (Animation_OnFrame(&this->skelAnime, 5.0f)) {
            Actor_PlaySfxAtProjectedPos(&this->actor, NA_SE_EN_FFLY_FLY);
        }
    }
