...
void EnBigokuta_Init(Actor* thisx, PlayState* play) {
    ...
    this->camAt.x = (Math_SinS(this->picto.actor.home.rot.y) * 66.0f) + this->picto.actor.world.pos.x;
...
void EnBigokuta_SetupCutsceneCamera(EnBigokuta* this, PlayState* play, Vec3f* at, Vec3f* eye) {
    ...
    this->camId = ActorCutscene_GetCurrentCamera(this->picto.actor.cutscene);
    Play_CameraSetAtEye(play, this->camId, at, eye);

    angle = BINANG_SUB(Actor_YawToPoint(&this->picto.actor, eye), this->picto.actor.home.rot.y);
    if (angle > 0) {
