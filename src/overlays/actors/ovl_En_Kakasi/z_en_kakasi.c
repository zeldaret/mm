...
void EnKakasi_Init(Actor* thisx, PlayState* play) {
    ...
    this->songSummonDist = KAKASI_GET_SUMMON_DISTANCE(&this->actor) * 20.0f;
    ...
    if (this->actor.world.rot.x > 0 && this->actor.world.rot.x < 8) {
        this->actor.targetMode = KAKASI_GET_TARGETMODE(thisx);
    }
    this->actor.shape.rot.y = this->actor.world.rot.y;

    this->aboveGroundStatus = KAKASI_GET_ABOVE_GROUND(&this->actor);
