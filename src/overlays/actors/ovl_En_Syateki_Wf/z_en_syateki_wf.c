...
void EnSyatekiWf_Init(Actor* thisx, PlayState* play) {
    ...
    for (i = 0; i < EN_SYATEKI_WF_GET_ID(&this->actor); i++) {
        path = &play->setupPathList[path->unk1];
    }
...
void func_80A201CC(EnSyatekiWf* this) {
    ...
    this->unk_2A4 = 1;
    this->unk_298 = 0;
    syatekiMan->wolfosFlags &= ~(1 << EN_SYATEKI_WF_GET_ID(&this->actor));
...
void func_80A20284(EnSyatekiWf* this, PlayState* play) {
    ...
    if (this->actor.parent != NULL) {
        ...
        if ((syatekiMan->shootingGameState == SG_GAME_STATE_RUNNING) && (this->unk_298 == 1)) {
            func_80A200E0(this);
            func_80A2030C(this);
        } else if (syatekiMan->wolfosFlags & (1 << EN_SYATEKI_WF_GET_ID(&this->actor))) {
            this->unk_298 = 1;
        }
    }
}
