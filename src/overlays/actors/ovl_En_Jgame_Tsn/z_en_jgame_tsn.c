...
void func_80C141DC(EnJgameTsn* this) {
    ...
    this->unk_2FC = 0;
    *this->unk_208[this->unk_218] |= OBJLUPYGAMELIFT_IGNITEFIRE;
...
void func_80C14230(EnJgameTsn* this, PlayState* play) {
    ...
    if ((this->unk_2FC > 100) || func_80C14BCC(this, play)) {
        ...
        for (i = 0; i < ARRAY_COUNT(this->unk_208); i++) {
            if (i == this->unk_218) {
                *this->unk_208[i] |= OBJLUPYGAMELIFT_IGNITEFIRE;
...
s32 func_80C14BCC(EnJgameTsn* this, PlayState* play) {
    ...
    if (player->actor.bgCheckFlags & 2) {
        ...
        if (phi_s3 == this->unk_218) {
            ...
            *this->unk_208[phi_s3] |= OBJLUPYGAMELIFT_DISPLAYCORRECT;
            play->interfaceCtx.unk_25C = 1;
            return true;
        }

        if (*this->unk_208[phi_s3] & OBJLUPYGAMELIFT_IGNITEFIRE) {
            Actor_PlaySfxAtPos(&this->actor, NA_SE_SY_TRE_BOX_APPEAR);
            *this->unk_208[phi_s3] |= OBJLUPYGAMELIFT_DISPLAYCORRECT;
            play->interfaceCtx.unk_25C = 1;
        } else {
            *this->unk_208[phi_s3] |= OBJLUPYGAMELIFT_DISPLAYINCORRECT;
