...
s32 func_8089AE00(EnDinofos* this, GlobalContext* globalCtx) {
    ...
    if (temp_v0 != NULL) {
        ...
        if (ABS_ALT(temp_v1) < 0x3000) {
            ...
            Actor_PlaySfxByPos2(&this->actor, NA_SE_EN_RIZA_WARAU);
            return true;
        }
    }
...
void func_8089B320(EnDinofos* this) {
    ...
    Actor_PlaySfxByPos2(&this->actor, NA_SE_EN_RIZA_CRY);
...
void func_8089B72C(EnDinofos* this, GlobalContext* globalCtx) {
    if (Animation_OnFrame(&this->skelAnime, 2.0f)) {
        Actor_PlaySfxByPos2(&this->actor, NA_SE_EN_RIZA_ATTACK);
    }
...
void func_8089C164(EnDinofos* this) {
    ...
    Actor_PlaySfxByPos2(&this->actor, NA_SE_EN_RIZA_JUMP);
...
void func_8089C87C(EnDinofos* this, s32 arg1) {
    ...
    this->unk_28E = 0;
    Actor_PlaySfxByPos2(&this->actor, NA_SE_EN_RIZA_DAMAGE);
    this->colliderJntSph.base.acFlags &= ~AC_ON;
...
void func_8089CFAC(EnDinofos* this) {
    ...
    this->actor.flags &= ~1;
    Actor_PlaySfxByPos2(&this->actor, NA_SE_EN_RIZA_DEAD);
    this->actor.speedXZ = 0.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y;
