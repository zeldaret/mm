...
typedef struct {
    /* 0x0 */ f32
        sprintSpeedMultiplier; // Target speed is multiplied by this when the dog is in the last 1/4 of the race
    /* 0x4 */ f32 goodConditionSpeedMultiplier; // Target speed is multiplied by this if the dog is in good condition
    /* 0x8 */ s16 color;                        // The dog's color, which is used as an index into sBaseSpeeds
    /* 0xA */ s16 index;                        // The dog's index within sDogInfo
    /* 0xC */ s16
        pointToUseSecondBaseSpeed; // When the dog is at or after this point, the second value in sBaseSpeeds is used
