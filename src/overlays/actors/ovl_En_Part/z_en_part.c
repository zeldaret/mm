...
void func_808654C4(EnPart* this, PlayState* play) {
    ...
    if (this->actor.params == PARAMS_15) {
        this->unk146--;
        if (this->unk146 > 0) {
        ...
        if (this->unk146 <= 0) {
            switch (this->actor.params) {
                case PARAMS_1:
                    func_800B3030(play, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, this->actor.scale.y * 4000.0f,
                                  7, 1);
                    SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 10, NA_SE_EN_EXTINCT);
                    break;
                case PARAMS_4:
                    for (i = 7; i >= 0; i--) {
                        ...
                        EffectSsDtBubble_SpawnColorProfile(play, &effectPos, &effectVelocity, &gZeroVec3f, effectScale,
                                                           25, 0, 1);
                    }
                    break;
            }
...
EnPartActionFunc D_8086590C[] = { func_80865390, func_808654C4 };
