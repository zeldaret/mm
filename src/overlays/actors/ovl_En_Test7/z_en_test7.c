...
#ifdef NON_MATCHING
// something with the bss accesses
...
void func_80AF118C(GlobalContext* globalCtx, EnTest7Struct2* arg1, EnTest7* this, s32 arg3, s32 arg4) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->unk_15C); i++) {
        ...
        if (arg3) {
            ...
            phi_s1 = -10000;
            if (temp_f0 > 400.0f) {
                phi_s1 = phi_s1 / (((temp_f0 - 400.0f) * 0.00125f) + 1.0f);
            }
        ...
        if (ptr[i].unk_00 != 0) {
            if ((ptr[i].unk_08.x > 30000.0f) || (ptr[i].unk_08.x < -30000.0f) || (ptr[i].unk_08.y > 30000.0f) ||
                ...
                if (temp_v0 != NULL) {
                    ...
                    gSPDisplayList(POLY_OPA_DISP++, D_04081628);
                }
            }
        }
    }
...
void EnTest7_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    func_80183430(&this->unk_18CC, &D_04085640, &D_04083534, this->unk_18FC, this->unk_1BA8, 0);
    func_801834A8(&this->unk_18CC, &D_04083534);
...
void func_80AF1E44(EnTest7* this, GlobalContext* globalCtx) {
    ...
    if (Rand_ZeroOne() < 0.3f) {
        camera = Play_GetCamera(globalCtx, ActorCutscene_GetCurrentCamera(globalCtx->unk_1879C[8]));
        rand = Rand_ZeroOne();
        sp34.x = ((camera->eye.x - this->actor.world.pos.x) * rand) + this->actor.world.pos.x;
        sp34.y = ((camera->eye.y - this->actor.world.pos.y) * rand) + this->actor.world.pos.y;
        sp34.z = ((camera->eye.z - this->actor.world.pos.z) * rand) + this->actor.world.pos.z;
...
void func_80AF29C0(EnTest7* this, GlobalContext* globalCtx) {
    static s16 D_80AF3450[2] = { 0, 0x31C7 };
    static f32 D_80AF3454 = 3500.0f;
    s32 pad;
    Player* player = GET_PLAYER(globalCtx);
    Vec3f* pos = &player->actor.world.pos;
    Camera* temp_s0 = Play_GetCamera(globalCtx, ActorCutscene_GetCurrentCamera(globalCtx->unk_1879C[8]));
...
void EnTest7_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->unk_144 & 2) {
        ...
        AnimatedMat_DrawStep(globalCtx, Lib_SegmentedToVirtual(&D_040815D0), sp40);
        Gfx_DrawDListXlu(globalCtx, D_04080FC8);
        Matrix_StatePop();
    }
