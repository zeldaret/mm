...
def handler_Array(symbol_name, offset, collection, reselem: Element):
    # TODO remove Static
    xml_errors.check_attrib(reselem, {"Name", "Count"}, {"Offset", "Static"})
    ...
    elif array_elem.tag == "Scalar" and array_elem.attrib["Type"] == "x8":
        array_resource_type = U8ArrayResourceDesc
    elif array_elem.tag == "Pointer" and array_elem.attrib["Type"] == "Gfx":
        array_resource_type = GfxPointerArrayResourceDesc
    elif array_elem.tag == "CollisionPoly":
        # TODO somehow handle this in z64resources.py instead
        from .z64resources import CollisionPolyArrayResourceDesc

        array_resource_type = CollisionPolyArrayResourceDesc
    else:
...
class CITextureResourceDesc(TextureResourceDesc):
    tlut: Optional[TextureResourceDesc]
...
def handler_Texture(
    ...
    if format.fmt == G_IM_FMT.CI:
        ...
        # TODO make all ci images have tlut info and enable this assert
        assert 1 or (
        ...
        if "Tlut" in reselem.attrib:
            xml_errors.check_attrib(
                ...
                # TODO remove OutName, SplitTlut
                {"OutName", "Offset", "SplitTlut", "HackMode"},
            )
        ...
        elif "ExternalTlutOffset" in reselem.attrib:
            xml_errors.check_attrib(
                reselem,
                {
                    "Name",
