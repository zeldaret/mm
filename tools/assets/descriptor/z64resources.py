...
def handler_Limb(symbol_name, offset, collection, reselem: Element):
    # TODO use LimbType consistently instead of Type
    xml_errors.check_attrib(
        reselem, {"Name"}, {"LimbType", "Type", "Offset", "EnumName"}
    )
    limb_type = LimbType[
        (
            reselem.attrib["LimbType"]
            if "LimbType" in reselem.attrib
            else reselem.attrib["Type"]
        ).upper()
    ]
...
@dataclasses.dataclass(eq=False)
class TextureAnimationResourceDesc(ResourceDesc):
    pass


def handler_TextureAnimation(
    symbol_name, offset, collection: ResourcesDescCollection, reselem: Element
):
...
@dataclasses.dataclass(eq=False)
class CollisionPolyArrayResourceDesc(ResourceDesc):
    count: int
