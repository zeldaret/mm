...
def register_resource_handlers():
    ...
    def ci_texture_resource_handler(
        ...
        def callback_set_tlut(pool_resources_by_desc):
            ...
            if resource_tlut_desc is not None:
    ...
    def collision_poly_list_resource_handler(
        file: File,
        resource_desc: z64resources.CollisionPolyArrayResourceDesc,
    ):
        return collision_resources.CollisionPolyListResource(
            file, resource_desc.offset, resource_desc.symbol_name, resource_desc.count
        )

    def animated_material_resource_handler(
        file: File, resource_desc: z64resources.TextureAnimationResourceDesc
    ):
        return animated_materials.AnimatedMaterialResource(
            file, resource_desc.offset, resource_desc.symbol_name
        )
