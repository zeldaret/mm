...
class ActorEntryListResource(CDataArrayNamedLengthResource):
    def write_elem(resource, memory_context, v, wctx: CDataExtWriteContext):
        ...
        try:
            ...
            # TODO I may be parsing garbage hence the need for this.
            actor_id_name = f'{v["id"]:#X}'
...
class SpawnListResource(CDataArrayResource):
    ...
    def try_parse_data(self, memory_context):
        ...
        # FIXME in oot we can guess an initial amount of spawns with the entrance table data
        # what about MM?
        num_spawns = 32
    ...
    def get_c_declaration_base(self):
        return f"EntranceEntry {self.symbol_name}[]"
...
class EnvLightSettingsListResource(CDataArrayNamedLengthResource):
    ...
    def get_h_includes(self):
        return (Z64HDRPRFX + "environment.h",)
...
class PathListResource(CDataArrayResource):
    ...
    def write_elem(
        ...
        f.write(", ")
        f.write(f"{v['additionalPathIndex']}, ")
        f.write(f"{v['customValue']}, ")
    ...
    elem_cdata_ext = (
        CDataExt_Struct(
            (
                ...
                ("additionalPathIndex", CDataExt_Value.u8),
                ("customValue", CDataExt_Value.s16),
