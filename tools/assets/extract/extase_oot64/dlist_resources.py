...
class TextureResource(Resource):
    ...
    def get_c_declaration_base(self):
        if hasattr(self, "HACK_IS_STATIC_ON") and self.is_tlut():
            if self.symbol_name in {"gPowderKegBarrelTLUT", "gPowderKegGoronSkullTLUT"}:
                return f"{self.elem_type} {self.symbol_name}[{(self.range_end - self.range_start)} / sizeof({self.elem_type})]"
            raise NotImplementedError
    ...
    def set_paths(self, extracted_path: Path, build_path: Path, out_path: Path):
        super().set_paths(extracted_path, build_path, out_path)
...
class TLUTResource(TextureResource, can_size_be_unknown=True):
    ...
    def __init__(self, file: File, range_start: int, name: str, fmt: G_IM_FMT):
        ...
        # These TLUTs live in a different file from the CI images they're for
        # so overrides are needed to include the right file (see TextureResource.set_paths)
        # however the TLUT object is also not involved in parsing the CI images
        # so it doesn't get an amount of colors like normal
        # But, the amount of colors must match between the TLUT object and the extracted TLUT in the other file
        # so we just hardcode the correct amount of colors here for simplicity
        if name in {
            "Z2_SONCHONOIE_00006F00_TLUT",
            "Z2_YADOYA_0000EA80_TLUT",
        }:
            fake_width = 252
            fake_height = 1
        if name in {
            "Z2_SONCHONOIE_00007100_TLUT",
            "Z2_SONCHONOIE_00007300_TLUT",
            "Z2_YADOYA_0000EE80_TLUT",
            "Z2_KAJIYA_000075D0_TLUT",
        }:
            fake_width = 256
            fake_height = 1
...
class ColorIndexedTexturesManager:
    ...
    def __init__(
        self,
        *,
        HACK_late_SetTextureLUT=False,
        HACK_ci4_ci8_mix=False,
        HACK_illegal_tlut_mode=False,
    ):
        ...
        # ci4+ci8 mix (e.g. multitexture)
        self.HACK_ci4_ci8_mix = HACK_ci4_ci8_mix

        # using CI textures with G_TT_NONE
        self.HACK_illegal_tlut_mode = HACK_illegal_tlut_mode
    ...
    def ci_timg(self, timg, fmt: G_IM_FMT, siz: G_IM_SIZ, width, height, pal):
        ...
        assert fmt == G_IM_FMT.CI
        if not self.HACK_late_SetTextureLUT and not self.HACK_illegal_tlut_mode:
    ...
    def tlut(self, tlut, idx, count):
        ...
        if self.cur_tluts_count != count:
            clear_cur_tluts_on_count_change = True
            if self.HACK_ci4_ci8_mix:
                clear_cur_tluts_on_count_change = False
            if clear_cur_tluts_on_count_change:
    ...
    def report_states(self, reporter: Resource, memory_context: "MemoryContext"):
        ...
        for ci_state in self.ci_states:
            ...
            for tex in ci_state.texs:
                ...
                if (
                    resource.name == "object_um_Tex_018710"
                    and resource_tlut.name == "object_um_TLUT_0132D0"
                ):
                    """
                    fake detection due to object_um_DL_00E6A8
                    loading some textures without drawing anything with it
                    """
                    continue

                if (
                    resource.name == "gTurtleShellScutesTex"
                    and resource_tlut.name == "gTurtleSkinTLUT"
                ):
                    """
                    fake detection due to gTurtleFrontLeftUpperFlipperDL
                    loading some textures without drawing anything with it
                    """
                    continue
...
class DListResource(Resource, can_size_be_unknown=True):
    ...
    def try_parse_data(self, memory_context):
        ...
        ci_tex_manager = ColorIndexedTexturesManager(
            ...
            HACK_ci4_ci8_mix=(
                self.name in {"Z2_MUSICHOUSE_room_00DL_00C770_0300A458_DL_03009A28_DL"}
            ),
            HACK_illegal_tlut_mode=(
                self.name in {"Z2_TENMON_DAI_room_01DL_00C848_03005140_DL_03003358_DL"}
            ),
        )
        ...
        def timg_cb(timg, fmt, siz, width, height, pal):
            ...
            assert width > 0, (f"{timg:#X}", width)
            assert height > 0, (f"{timg:#X}", height)
