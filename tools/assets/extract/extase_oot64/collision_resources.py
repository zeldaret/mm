...
from ..extase.memorymap import UnexpectedResourceTypeError
        ...
        for i_data, bitfield_info in (
            ...
            for mask, shift, name, fmt_info in bitfield_info:
                try:
                    ...
                    raise Exception(f"could not write surface type field {name}") from e
...
class CollisionResource(CDataResource):
    def write_numVertices(
        ...
        wctx.f.write(wctx.line_prefix)
        if resource.cdata_unpacked["vtxList"] == 0:
            wctx.f.write(f"{v}")
        else:
            wctx.f.write(
    ...
    def report_vtxList(
        ...
        try:
            ...
            ignore_exc = False
            if resource.name == "sTurtleGreatBayTempleCol":
                # TODO this deals with some jank due to the vertex arrays using <Array> of vec3s but collision Vec3s vertices arrays are CollisionVtxListResource
                # proper fix would be replace CollisionVtxListResource usage with Vec3sArrayResource
                ignore_exc = True
            if not ignore_exc:
                raise
    ...
    def write_numPolygons(
        ...
        wctx.f.write(wctx.line_prefix)
        if resource.cdata_unpacked["polyList"] == 0:
            wctx.f.write(f"{v}")
        else:
            wctx.f.write(
                memory_context.get_c_expression_length_at_segmented(
    ...
    def try_parse_data(self, memory_context):
        ...
        if surfaceTypeList_address != 0:
            ...
            if self.resource_polyList.is_data_parsed:
                ...
                length_surfaceTypeList = (
                    self.resource_polyList.max_surface_type_index + 1
                )
        ...
        if surfaceTypeList_address != 0:
            ...
            # TODO guess length_bgCamList better in case surfaceTypeList is NULL
            length_bgCamList = 1
            self.resource_bgCamList = memory_context.report_resource_at_segmented(
                self,
                bgCamList_address,
        ...
        assert self.resource_bgCamList is not None
        return RESOURCE_PARSE_SUCCESS
