...
class File:
    ...
    def set_source_path(self, source_path: Path):
        ...
        if file_name.endswith(".unarchive"):
            # HACK
            file_name = file_name.removesuffix(".unarchive")
    ...
    def write_source(self):
        ...
        with self.source_c_path.open("w") as c:
            with self.source_h_path.open("w") as h:
                ...
                included_headers_in_c.update(global_includes)
                included_headers_in_h.update(global_includes)
                ...
                for resource in self._resources:
                    ...
                    try:
                        ...
                        print("Error during write of", resource)
                        raise
...
class BinaryBlobResource(Resource):
    ...
    def get_c_declaration_base(self):
        if hasattr(self, "HACK_IS_STATIC_ON"):
            return f"u8 {self.symbol_name}[{self.range_end - self.range_start}]"
