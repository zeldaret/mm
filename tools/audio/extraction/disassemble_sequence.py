...
class ArgInstr(ArgU8):
    def emit(self, disas):
        ...
        if self.value in font0.instrument_index_map:
            ...
            print(f"[{disas.seq_name}] Invalid instrument sourced from {font0.name}: {self.value}")
    ...
    def read(self, disas):
        ...
        if ret & 0x80:
            ...
            if ret < 128 and disas.insn_begin not in disas.force_long:
                print(f"[{disas.seq_name}] Unnecessary use of long immediate encoding @ 0x{disas.insn_begin:X}: {ret}")
