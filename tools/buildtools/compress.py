...
# romStart equals this value indicates the segment is unset in the DMA table
DMA_ROM_UNSET = 0xFFFFFFFF
...
def compress_rom(
    ...
    with multiprocessing.Pool(n_threads, initializer=set_sigint_ignored) as p:
        ...
        for entry_index, dmadata_offset in enumerate(
            ...
            is_unset = segment_rom_start == DMA_ROM_UNSET
            is_compressed = (entry_index in compress_entries_indices) and not is_unset
    ...
    for segment in compressed_rom_segments:
        ...
        compressed_rom_dma_entries.append(
            DmaEntry(
                segment.vromStart,
                segment.vromEnd,
                segment_rom_start if not segment.is_empty else DMA_ROM_UNSET,
                segment_rom_end if segment.is_compressed else 0 if not segment.is_empty else DMA_ROM_UNSET,
            )
        )
