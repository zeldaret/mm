...
```
(from the root directory of `mm`). We can tell mips2c to use the context file we just generated by adding `--context ctx.c`. If we have data, mips2c may be able to asist with that as well.

In this case, we want the file for `EnRecepgirl_Init`. You can copy the path to the file in VSCode or similar, or just tab-complete it onec you know the directory structure well enough: it turns out to be `asm/non_matchings/overlays/ovl_En_Recepgirl/EnRecepgirl_Init.s`.
...
There are two different sorts of SkelAnime, although for decompilation purposes there is not much difference between them. From `SkelAnime_InitSV` we can read off that

- The `SkelAnime` struct is at `this + 0x144`
- The `jointTable` is at `this + 0x188`
- The `overrideDrawTable` is at `this + 0x218`
- The number of limbs is `0x18 = 24` (we use dec for the number of limbs)
- Hence the `jointTable` and `overrideDrawTable` both have `24` elements
...
The last information we get from the SkelAnime functions is the types of two of the externed symbols: `D_06011B60` is a `FlexSkeletonHeader`, and `D_06009890` is an `AnimationHeader`. So we can change/add these at the top of the C file:
...
```
note that `this->jointTable` and `this->morphTable` are 
...
In this case, we are looking for `this + 0x2AC`. `0x2AC` is not a very common number, so hopefully the only mention of it is in referring to this struct varible. Indeed, if we search the file, we find that the only instruction mentioning `0x2AC` is here:
```mips
...
3. This is probably a setup function for an actionFunc, with are usually either `void (*)(ActorType*)` or `void (*)(ActorType*, GlobalContext*)`.
