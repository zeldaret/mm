...
We use a [Google Sheet](https://docs.google.com/spreadsheets/d/1X83YCPRa532v-Zo0WgUsJ2kB1X9RxBta5_p9aWA8uro/edit#gid=0>) to keep track of all the files worked on and their merge status. On this board one can reserve a file to work on. To reserve a file, put your github/discord name in the "Reserved" column. It is common courtesy to not work on a file that is being worked on by another contributor, so ensure the "Reserved" column is blank before working on a file. If you really want to work on a specific file and it is reserved you can put your name in the "Interested" column then reach out to the individual who has reserved the file and to see if they are still interested in pursuing the file. Other columns of interest can be the "Size" columns. The smaller the size, generally means it is easier to decompile. We recommend new comers to work on the smaller sized files(Roughly under the Average size of 40). After doing a couple we encourage you to move on to moderate sized files as to maintain smaller files for other new contributors.
