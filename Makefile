...
ifneq ($(RUN_CC_CHECK),0)
  CHECK_WARNINGS := -Wall -Wextra -Wno-unknown-pragmas -Wno-unused-parameter -Wno-unused-variable -Wno-missing-braces -Wno-pointer-to-int-cast -Wno-unused-but-set-variable -Wno-unused-label
  CC_CHECK   := gcc -fno-builtin -fsyntax-only -funsigned-char -fdiagnostics-color -std=gnu89 -D _LANGUAGE_C -D NON_MATCHING $(IINC) -nostdinc
  ifneq ($(WERROR), 0)
    CC_CHECK += -Werror
  endif
else
...
build/src/code/audio/%.o: CHECK_WARNINGS += -Wno-int-conversion
